...
import { Api_Endpoint, updateQualification } from '../../../../../services/ApiCalls'
...
const Qualification = () => {
  ...
  const loadData = async () => {
    setLoading(true)
    try {
      const response = await axios.get(`${Api_Endpoint}/Qualifications/tenant/${tenantId}`)
  ...
  const { isLoading, mutate } = useMutation(updateQualification, {
    onSuccess: (data) => {
      queryClient.setQueryData(['qualification', tempData.id], data);
      reset()
      setTempData({})
      loadData()
      setIsUpdateModalOpen(false)
  ...
  const url = `${Api_Endpoint}/Qualifications`
